...
describe('blairTable', () => {
	...
	describe('能夠正確切換分頁模組', () => {
		it('分頁模組: classic', async () => {
			...
			const paginationDom = wrapper.find(`[data-name="blair__pagination__list"]`);
		...
		it('分頁模組: modern', async () => {
			...
			const classicPagiDom = wrapper.find(`[data-name="blair__pagination__list"]`);
			const haveBtnLen = wrapper.findAll('button').length;
			const pageInfoDom = wrapper.find(`[data-name="pagination__info"]`);
	...
	it('第一列表頭資料固定至上方', async () => {
		...
		expect(wrapper.find('.table-scrollbar').classes()).toContain('text-main-color-darkoverflow-visible');
